...
from Common.handle_data import replace_case_by_regular,EnvData
from Common.handle_requests import req

excel_path = os.path.join(datas_dir, "eleme_cases.xlsx")
get_excel = HandleExcel(excel_path, "获取所选城市信息")
cases = get_excel.read_all_datas()
get_excel.close_file()


@ddt
class TestGetSelectCityInfo(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        logger.info("***************************获取所选城市信息用例开始*****************************************")

    @classmethod
    def tearDownClass(cls) -> None:
        logger.info("***************************获取所选城市信息用例结束*****************************************")

    @data(*cases)
    def test_get_select_city_info(self,case):
        ...
        if case["expected"]:
            ...
            try:
                if 'message' in expected.keys():
                    ...
                    self.assertEqual(resp.json()["id"], expected["id"])
